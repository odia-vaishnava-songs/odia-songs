...
const newSongContent = `export const SONG_BOLOHARIBOLO_STRUCTURED: StructuredSong = {
    "verses": [
        {
            "id": 1,
        ...
        },
        {
            ...
            "wordMeanings": [
                ...
                { "word": "କୃଷ୍ଣେର ସୋଂସାରେ", "meaning": "କୃଷ୍ଣଙ୍କର ସଂସାରରେ" },
...
const regex = /export const SONG_BOLOHARIBOLO_STRUCTURED: StructuredSong = \{[\s\S]*?\};/;

if (regex.test(content)) {
    const newContent = content.replace(regex, newSongContent);
    fs.writeFileSync(filePath, newContent, 'utf8');
    console.log('Successfully updated SONG_BOLOHARIBOLO_STRUCTURED with user provided details');
} else {
    console.error('Could not find SONG_BOLOHARIBOLO_STRUCTURED block');
    process.exit(1);
}
